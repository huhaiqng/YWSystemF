...
<script>
import { getHosts, getProjectPython, addProjectPython, updateProjectPython, deleteProjectPython } from '@/api/resource'
...
export default {
  ...
  methods: {
    getList() {
      getProjectPython(this.queryList).then(response => {
        this.list = response
      })
    },
    resetTemp() {
    ...
    createData() {
      addProjectPython(this.temp).then(() => {
        this.$notify({
          title: '成功',
          message: '新增成功！',
          type: 'success',
    ...
    updateData() {
      updateProjectPython(this.temp).then(() => {
        this.getList()
        this.dialogVisible = false
        this.$notify({
          title: '成功',
