...
<script>
import { getProjectKafka, addProjectKafka, updateProjectKafka, deleteProjectKafka } from '@/api/resource'
export default {
  props: {
  ...
  methods: {
    getList() {
      getProjectKafka(this.queryList).then(response => {
        this.list = response
      })
    },
    resetTemp() {
    ...
    createData() {
      addProjectKafka(this.temp).then(() => {
        this.$notify({
          title: '成功',
          message: '新增成功！',
          type: 'success',
    ...
    updateData() {
      updateProjectKafka(this.temp).then(() => {
        this.getList()
        this.dialogVisible = false
        this.$notify({
          title: '成功',
