...
<script>
import { getHosts, getProjectWar, addProjectWar, updateProjectWar, deleteProjectWar } from '@/api/resource'
...
export default {
  ...
  methods: {
    getList() {
      getProjectWar(this.queryList).then(response => {
        this.list = response
      })
    },
    resetTemp() {
    ...
    updateData() {
      updateProjectWar(this.temp).then(() => {
        this.getList()
        this.dialogVisible = false
        this.$notify({
          title: '成功',
