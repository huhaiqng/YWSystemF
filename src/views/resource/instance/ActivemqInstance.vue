  ...
  <div class="app-container">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogVisible">
      <el-form ref="dataForm" :model="temp" label-position="left" label-width="100px" style="margin-right:30px; margin-left:30px;">
        ...
        <el-form-item label="Web 地址" prop="outside_addr">
    ...
    <el-drawer title="详情" :visible.sync="drawerVisible" :with-header="false">
      <activemq-instance-drawer :instance="instance" />
    </el-drawer>
  </div>
</template>

<script>
import { addActivemqInstance, deleteActivemqInstance, updateActivemqInstance, getActivemqInstance } from '@/api/instance'
import Pagination from '@/components/Pagination'
import ActivemqInstanceDrawer from '@/components/Drawer/ActivemqInstance'
export default {
  name: 'ActivemqInstance',
  components: { Pagination, ActivemqInstanceDrawer },
  data() {
    return {
      list: null,
      total: 0,
  ...
  methods: {
    getList() {
      getActivemqInstance(this.queryList).then(response => {
    ...
    handleDelete(id) {
      this.$confirm('确认删除', '提示', {
        ...
        deleteActivemqInstance(id).then(() => {
          this.$notify({
            title: '成功',
            message: '删除成功！',
            type: 'success',
    ...
    createData() {
      addActivemqInstance(this.temp).then(response => {
        this.getList()
    ...
    updateData() {
      updateActivemqInstance(this.temp).then(() => {
        this.getList()
        this.dialogVisible = false
        this.$notify({
          title: '成功',
